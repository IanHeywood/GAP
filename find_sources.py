...
def tiggerConvert(gaul):
	args = []
	tigger_convert  = x("/home/ianh/Software/MeqTrees/tigger/Tigger/bin/tigger-convert")
...
fitslist = glob.glob('*'+pattern+'*-MFS-image.fits')

print fitslist

for infits in fitslist:
	find_sources(infits)
	tiggerConvert(infits.replace('.fits','.pybdsm.gaul'))
